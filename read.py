...
def ERV(TextParser, read_sections_list, schema, code_tables_path):
    ...
    for i_chunk, string_df in enumerate(TextParser):
        ...
        [data_df, valid_df, out_dtypes ] = read_sections.main(
            sections_df, schema)
        # 3. Validate data elements
        valid_df = validate.validate(
            data_df, valid_df, schema, code_tables_path)
        # 4. Save to buffer
        # Writing options from quoting on to prevent data with 
        # special characters, like commas, etc, to be quoted
        #https://stackoverflow.com/questions/21147058/pandas-to-csv-output-quoting-issue
        data_df.to_csv(
            data_buffer,header = False, mode = 'a', encoding = 'utf-8',
            index = False, quoting = csv.QUOTE_NONE, 
            sep = properties.internal_delimiter,
            quotechar='\0', escapechar='\0')
        valid_df.to_csv(valid_buffer, header = False, mode = 'a',
    ...
    chunksize = TextParser.orig_options['chunksize'] if isinstance(
        TextParser, pd.io.parsers.TextFileReader) else None
    ...
    data = pd.read_csv(data_buffer, names=data_df.columns,
                       ...
                       quotechar='\00',escapechar='\00')
    valid = pd.read_csv(valid_buffer, names=data_df.columns,
                        chunksize=chunksize)
...
def main(source, data_model = None, data_model_path = None, sections = None,
         chunksize = None, skiprows = None, out_path = None):
    """
    ...
    sections : list, optional
        List with subset of data model sections to output (default is
        all)
    ...
    TextParser = import_data.main(
        ...
        quotechar=schema['header'].get('quotechar'))
    ...
    data_columns = [x for x in data] if isinstance(data, pd.DataFrame) \
    ...
    if out_path:
        ...
        for i, (data_df, valid_df) in enumerate(zip(data, valid)):
            ...
            kwargs = {
                'header' : header, 'mode' : mode, 'encoding' : 'utf-8',
