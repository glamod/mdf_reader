...
for path in paths_files:
    ...
    rig = (
        data.data[["c99_voyage"]]
        .c99_voyage.Ship_type.value_counts(dropna=False)
        .to_frame()
    )
    lat_ind = (
        data.data[["c99_voyage"]]
        .c99_voyage.LatInd.value_counts(dropna=False)
        .to_frame()
    )
    lon_ind = (
        data.data[["c99_voyage"]]
        .c99_voyage.LonInd.value_counts(dropna=False)
        .to_frame()
    )
    ...
    at_unit = (
        data.data[["c99_data"]]
        .c99_data.AT_reading_units.value_counts(dropna=False)
        .to_frame()
    )
    sst_unit = (
        data.data[["c99_data"]]
        .c99_data.SST_reading_units.value_counts(dropna=False)
        .to_frame()
    )
    ap_unit = (
        data.data[["c99_data"]]
        .c99_data.AP_reading_units.value_counts(dropna=False)
        .to_frame()
    )
    bart_unit = (
        data.data[["c99_data"]]
        .c99_data.BART_reading_units.value_counts(dropna=False)
        .to_frame()
    )
    lon_unit = (
        data.data[["c99_data"]]
        .c99_data.Longitude_units.value_counts(dropna=False)
        .to_frame()
    )
    baro_type = (
        data.data[["c99_data"]].c99_data.BARO_type.value_counts(dropna=False).to_frame()
    )
    Distance_unit = (
        data.data[["c99_data"]]
        .c99_data.Distance_units.value_counts(dropna=False)
        .to_frame()
    )
    Distance_unit_land = (
        data.data[["c99_data"]]
        .c99_data.Distance_units_to_landmark.value_counts(dropna=False)
        .to_frame()
    )
    Distance_unit_travel = (
        data.data[["c99_data"]]
        .c99_data.Distance_units_travelled.value_counts(dropna=False)
        .to_frame()
    )
    units_of_measures = (
        data.data[["c99_data"]]
        .c99_data.units_of_measurement.value_counts(dropna=False)
        .to_frame()
    )
    humidity_u = (
        data.data[["c99_data"]]
        .c99_data.humidity_units.value_counts(dropna=False)
        .to_frame()
    )
    release = (
        data.data[["c99_data"]].c99_data.Release.value_counts(dropna=False).to_frame()
    )
...
d["units_of_other_measurements"] = pd.concat(units_of_other_measurements, axis=1).sum(
    axis=1
)
